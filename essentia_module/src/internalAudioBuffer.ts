export default class InternalAudioBuffer {
    ...
    private availableWriteInternal(rd, wr) {
        let rv = rd - wr - 1
        if (wr >= rd) {
    ...
    private availableReadInternal(rd, wr) {
        if (wr > rd) {
            return wr - rd
        } else {
